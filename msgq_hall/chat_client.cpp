// Compile : g++ -std=c++17 chat_client.cpp -o chat_client.exe -luser32 -lgdi32
// ./chat_client.exe
...
#define CMD_LEAVE_ROOM   105 // (เพิ่มใหม่)
        ...
        case WM_COPYDATA: {
            ...
            switch (cds->dwData) {
                ...
                case RSP_JOIN_SUCCESS:
                    ...
                    if (message.empty()) {
                        // Server ยืนยันว่ากลับมา Lobby (อาจจะตอน Register หรือ Leave)
                        std::cout << "[CLIENT] Returned to Lobby." << std::endl;
                    } else {
                        // Server ยืนยันว่าเข้าห้องสำเร็จ
                        std::cout << "[CLIENT] Successfully joined room '" << message << "'.\n";
                    }
                    break;
            }
...
bool SendCommand(DWORD commandType, const std::string& payload = "") {
    ...
    cds.cbData = (DWORD)payload.length() + 1;
...
int main() {
    // --- (เพิ่มใหม่) ---
    // แก้ภาษาเพี้ยนใน Console (ต้องเรียกก่อนพิมพ์อะไร)
    ...
    while (g_running) {
        ...
        // คำสั่ง /leave
            ...
            if (g_currentRoom.empty()) {
                std::cout << "[ERROR] You are in the lobby. Use /join <room_name> to chat.\n";
            } else {
